...
set_t *set_union(const set_t *set1, const set_t *set2) {
    ...
    iter = red_black_tree_iterator_create(set1->tree);
    return_value_if_fail(iter != NULL, NULL);
    while (red_black_tree_iterator_has_next(iter)) {
        elem = red_black_tree_iterator_next(iter);
        set_add(union_set, elem);
    }
    red_black_tree_iterator_destroy(iter);

    iter = red_black_tree_iterator_create(set2->tree);
    return_value_if_fail(iter != NULL, NULL);
    while (red_black_tree_iterator_has_next(iter)) {
        elem = red_black_tree_iterator_next(iter);
        set_add(union_set, elem);
...
set_t *set_intersection(const set_t *set1, const set_t *set2) {
    ...
    return_value_if_fail(set1 != NULL && set2 != NULL, NULL);
...
bool set_is_subset(const set_t *set1, const set_t *set2) {
    ...
    return_value_if_fail(set1 != NULL, true);
    return_value_if_fail(set2 != NULL, false);
